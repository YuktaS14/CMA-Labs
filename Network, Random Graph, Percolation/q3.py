...
class UndirectedGraph():
    ...
    def bfs(self,source):
        ...
        queue = [];
        queue.append(source);
        # we add at the end of the list and pop from 0th index to implement FIFO
        while len(queue) != 0:
            source = queue.pop(0);
            ...
            for nbr in self.adjacencyList[source]:
                if not self.visited[nbr]:
                    queue.append(nbr);
                    # update the visited of new source popped from the queue 
                    self.visited[nbr] = 1;
...
# function to verify the connectedness of a random graph based on the probability 
def ERGraphConnectivity():
    print("Erd ̋os-R ́enyi random graph G(100, p) is almost surely connected only if p > ln 100/100.\n AS can be visualized from the graph: \n")
    g = ERRandomGraph(100);
    # 50 probabilities are taken between 0 and 0.1 and is stored in x
    x = np.linspace(0,0.1,50);
    # print(x);
    # threshold is taken as per statement
    x_threshold = math.log(100)/100;
    # we average the fraction of graphs connected over 1000 runs
    noOfRuns = 1000;
    y = [];

    # for each probability in x we create a sample graph 1000 times, and get the average number of times the graph is connected
    for i in x:
        count = 0;
        for j in range(noOfRuns):
            g.sample(i);
            # each time a graph is found to be connected, we increment the count
            if g.isConnected():
                count+=1;
        #append the value to y for corresponding x_probability
        y.append(count/noOfRuns);
    
    #plotting the graph verifying the statement
    plt.plot(x,y,'b');
    plt.axvline(x_threshold,color='r',label = "Theoretical threshold");
    plt.grid();
